...
class Vec4(ctypes.Structure):
    _fields_ = Vec3._fields_ + [ ("w", ctypes.c_float) ]
...
BodyCallback = ctypes.CFUNCTYPE(None, ctypes.POINTER(Body), ctypes.c_uint, ctypes.POINTER(Vec4))
ColorCallback = ctypes.CFUNCTYPE(None, ctypes.POINTER(Image))
...
def make_body_callback(payload):
    ...
    def callback(data, size, floor):
        ...
        for i in range(size):
            ...
            body[body_i.index] = pose

        payload['body'] = body

        floor = floor.contents
        payload['floor'] = [ floor.x, floor.y, floor.z, floor.w ]
...
def make_color_callback(payload):
    ...
    def callback(img_p):
        ...
        pixel = ctypes.c_ubyte * 4
        row = pixel * img.width
        image = row * img.height

        assert img.data is not None

        ptr = ctypes.cast(img.data, ctypes.POINTER(ctypes.c_ubyte))
        ptr = image.from_address(ctypes.addressof(ptr.contents))
        
        shape = (img.height, img.width, 4)
        
        payload['color'] = np.ctypeslib.as_array(ptr)
